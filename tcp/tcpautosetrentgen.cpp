...
//            TCPCommandSeparatorOnFull *onCommand = new TCPCommandSeparatorOnFull(mainModel->getServer(), mainModel, getTimeOutWidget(), true);
            TCPGetSpectrumsGistogramms *getSpectrum = new TCPGetSpectrumsGistogramms(mainModel->getServer(), getspk, mainModel, 10, threads, getTimeOutWidget(), getLogWidget());
            getSpectrum->setWithOffExp(false);
            getSpectrum->setWithRGU(true);
